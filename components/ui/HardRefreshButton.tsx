...
export function HardRefreshButton({ className }: { className?: string }) {
  ...
  return (
    <Pressable
      ...
      className={`h-10 w-10 items-center justify-center rounded-full border border-surface-border bg-white/5 backdrop-blur-md transition-[transform,opacity] duration-150 ease-in-out active:scale-95 active:opacity-80 ${className ?? ''}`}
    >
      <Animated.View style={{ transform: [{ rotate }] }}>
        <RefreshCw color="#A1A1AA" size={18} />
      </Animated.View>
    </Pressable>
  );
}
